...
function plot(path::Union(GLMNetPath, CoxNetPath), args...; x = :lambda, y = :coefficients, color = :variable, xgroup = :response, kw...)
	...
	elseif y == :absCoefficients
		df[:absCoefficients] = abs(betas[:])
	else 
		error("y must be either :coefficients or :absCoefficients.")
	end
	...
	elseif x == :norm2
		df[:nrom2] = rep(sqrt(sum(abs(betas).^2, 1))[:], each = nvar)
	else 
		error("x must be either :lambda, :deviance, :norm1, :norm2.")
	end
